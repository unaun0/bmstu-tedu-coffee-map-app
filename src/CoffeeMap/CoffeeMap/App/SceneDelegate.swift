...
class SceneDelegate: UIResponder, UIWindowSceneDelegate {
    ...
    func scene(
        ...
        let useCase = AllCoffeeShopsUseCase(
        ...
        let presenter = AllCoffeeShopsPresenter(useCase: useCase)
        let allCoffeeVC = AllCoffeeShopsViewController(presenter: presenter)

        presenter.view = allCoffeeVC

        let allCoffeeTab = AllCoffeeTab(tag: 0, viewController: allCoffeeVC)
        let favoritesTab = FavoritesTab(tag: 1)
